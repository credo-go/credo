...
- [x] Define `Context` struct with core methods ([ADR-008](docs/adr/008-context-design.md))
  ...
  - [x] Strict JSON bodies + typed decode errors: exactly one JSON value per body (trailing data rejected), decode failures return `*BindError` with machine-readable reasons (`syntax`, `type_mismatch`, `invalid_value`, `empty_body`, `trailing_data`) rendered as RFC 7807 `errors[]` (code = reason, i18n via `bind.<reason>`)
