...
- **Data access (`store/sqldb`)** — typed terminal methods on `*SelectQuery` (Go 1.27 concrete-type generic methods): `One[T](ctx) (T, error)` and `All[T](ctx) ([]T, error)` return the queried type directly instead of scanning into a caller-supplied destination. `T` drives both the table and the destination, so the query is built model-less (`db.Select().Where(...).One[User](ctx)`). `One` applies `LIMIT 1` (first row; multiple matches are not an error — add `OrderExpr` for determinism) and maps no-row to `store.ErrNotFound`; `All` returns a non-nil empty slice with a nil error when nothing matches. Both clone the query and inject the ambient transaction exactly like `Scan`. See [ADR-015](docs/adr/015-data-access.md).
- **Data access (`store/sqldb`)** — `(*SelectQuery).Page[T](ctx, req) (*pagination.Page[T], error)`, a typed pagination terminal completing the `One`/`All`/`Page` family. It runs COUNT + a LIMIT/OFFSET SELECT and returns a ready `*pagination.Page[T]`; `T` drives the table, so the query is built model-less (`db.Select().Where(...).OrderExpr(...).Page[User](ctx, req)`). `req` is assumed already normalized (`BindQuery` does this via `Validate`) and is not re-normalized; a nil `req` is rejected; on zero rows the SELECT is skipped and the page preserves the requested page/per-page with a non-nil empty slice. COUNT and SELECT both join the ambient transaction. See [ADR-015](docs/adr/015-data-access.md).
- **Data access (`store`)** — `*store.TxScope` now has method forms of the scoped transaction helpers (Go 1.27 concrete-type generic methods): `scope.WithTx[T](ctx, tx)`, `scope.GetTx[T](ctx)`, and `scope.Conn[T](ctx, fallback)`. They are additive ergonomic sugar over the existing `WithTxInScope` / `GetTxInScope` / `ConnInScope` free functions — same context keying, so the two forms are interchangeable — and keep the scope on the value the caller already holds. The unscoped `store.WithTx` / `GetTx` / `Conn` stay free functions because their receiver would be `context.Context`. See [ADR-015](docs/adr/015-data-access.md).
...
- **BREAKING — `sqldb.Paginate` and `sqldb.PaginateRequest` are removed.** The typed `(*SelectQuery).Page[T]` terminal replaces `PaginateRequest` one-for-one: `sqldb.PaginateRequest[User](ctx, q, req)` → `q.Page[User](ctx, req)`. For a model→DTO response (where the old low-level `Paginate` filled a caller-supplied slice), build the page from the terminals instead: `q.Clone().Model((*Model)(nil)).Count(ctx)` + `q.Clone().Offset(req.Offset()).Limit(req.PerPage).All[Model](ctx)` + `pagination.NewPage(dtos, total, req.Page, req.PerPage)`. The ORM-agnostic `pagination` types (`PageRequest`, `Page[T]`, `NewPage`, `Meta`, `SortRequest`) are unchanged. See [ADR-015](docs/adr/015-data-access.md).
