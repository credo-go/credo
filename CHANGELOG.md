...
- **BREAKING (Go API): `App.Mux()` removed** — `*App` satisfies the `Routes` introspection interface directly; call `credo.Walk(app, fn)` / `credo.WalkRoutes(app, fn)` instead of passing `app.Mux()`.
- **BREAKING (Go API): `auth.Middleware` panics on a nil authenticator** — a missing authenticator is a wiring error and now fails fast at construction (`auth: Middleware requires a non-nil authenticator`) instead of turning every request into a 401. The reflection-based typed-nil probe and the `auth.ErrAuthenticatorRequired` sentinel are removed.
- **BREAKING (behavior): CORS `AllowOrigins` uses the strict shared origin grammar** — entries are parsed with the same rules as the websocket adapter's `AllowedOrigins`: `scheme://host[:port]`, `http`/`https` only, no path, query, fragment, or userinfo, case-insensitive host, implied default port, and at most one wildcard standing for exactly the left-most DNS label (`https://*.example.com` matches `https://app.example.com` but not `https://example.com` or `https://a.b.example.com`). The former prefix/suffix glob (a `*` anywhere, e.g. `https://api-*-prod.example.com`) and blank entries are gone; invalid entries panic when `middleware.CORS` is built. A matched origin is echoed back in the request's own serialization, and a malformed `Origin` header never matches. `"*"` and `AllowOriginFunc` are unchanged.
...
- **Placeholder directories** — `observability/`, `pubsub/`, `grpc/`, and `openapi/` held only a README marker and are removed from the module; planned areas exist solely as roadmap entries in `TODO.md` until real code lands.
...
- **Redacted config formatting** — `*config.Config` now implements `String()`, `GoString()`, and `slog.LogValuer` with a metadata-only description (`config.Config(N keys, values redacted)`), so formatting or logging a Config can never leak configuration values or key names.
- **Reload-signal opt-out** — `credo.WithoutReloadSignals()` disables SIGHUP-triggered reloads under `Run()` with subscribe-and-ignore semantics: the signal is still captured, so a stray SIGHUP can never terminate the process, but it is logged at Info (`credo: reload signal ignored (reload signals disabled)`) instead of triggering `App.Reload`. SIGINT/SIGTERM shutdown handling and programmatic `App.Reload` are unaffected; no-op on Windows. Raw Unix signal disposition remains available via `RunContext`/`ServeContext`.
- **Strict typed decoding** — `config.WithStrictDecoding()` makes every decode from the store reject unknown keys (nested included, error names the key path) and disables weak string coercion (`"123"`→int, `"true"`→bool), while duration strings, `encoding.TextUnmarshaler` fields, and native numeric conversions keep working. The policy covers `Unmarshal`/`Get`, `Stage`/reload subscriber validation, and the framework's own `server` section decode in `credo.New` (an unknown `server.*` key fails construction). Designed for typed sources: string env/.env overrides on typed fields do not decode under strict mode, and typed views must cover their section completely. Unknown-key errors never contain config values; type-mismatch errors may quote the offending value. Default remains weak decoding.
