...
func TestProvideProtectedValue_RejectsReplace(t *testing.T) {
	...
	if _, _, err := c.Replace[*SimpleService](&SimpleService{Value: "replacement"}); err == nil {
...
func TestProtectBinding(t *testing.T) {
	...
	t.Run("mismatched expected value leaves binding replaceable", func(t *testing.T) {
		...
		c.MustProvideValue[*SimpleService](original)
		if old, existed, err := c.Replace[*SimpleService](replacement); err != nil || !existed || old != original {
			t.Fatalf("Replace() = (%p, %v, %v), want (original %p, true, nil)", old, existed, err, original)
		}
		if err := c.ProtectBinding[*SimpleService](original); err == nil {
			t.Fatal("ProtectBinding(expected) should reject a changed value")
		}
		if _, _, err := c.Replace[*SimpleService](&SimpleService{Value: "final"}); err != nil {
	...
	t.Run("unresolved expected value leaves binding replaceable", func(t *testing.T) {
		...
		if _, existed, err := c.Replace[*SimpleService](&SimpleService{}); err != nil || existed {
			t.Fatalf("Replace() over an unbuilt constructor = (existed %v, %v), want (false, nil)", existed, err)
		}
	})
	...
	t.Run("non-comparable expected value leaves binding replaceable", func(t *testing.T) {
		...
		if _, _, err := c.Replace[[]string]([]string{"replacement"}); err != nil {
