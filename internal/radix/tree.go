...
// already has an explicit (non-auto-generated) endpoint. Existing is the
// payload of the prior registration, so a caller that knows V's concrete type
// can surface registration-site detail (e.g. the original file:line) that the
// generic tree itself cannot read.
