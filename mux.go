...
func (mx *mux) insert(method, pattern string, rh *routeHandler, autoGenerated ...bool) {
	...
	if err != nil {
		if dup, ok := errors.AsType[*radix.DuplicateRouteError[*routeHandler]](err); ok {
			panic(duplicateRoutePanic(method, pattern, rh, dup.Existing))
		}
...
func duplicateRoutePanic(method, pattern string, incoming, existing *routeHandler) string {
...
// already registered at that exact pattern — and returns the conflicting
// payload so the caller can build a descriptive panic. It is the read-only
...
func (mx *mux) wouldConflict(method, pattern string) (*routeHandler, bool) {
	mtyp, ok := radix.LookupMethod(method)
	if !ok {
		panic(fmt.Sprintf("credo: unknown method %q", method))
	}
