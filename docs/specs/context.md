...
`Response` implements `io.ReaderFrom`, so `Stream` and any `io.Copy` into the response copy a Reader-only source without allocating a buffer per call. When the underlying `http.ResponseWriter` implements `io.ReaderFrom` (net/http's HTTP/1.1 writer, which uses its own pooled buffer and sendfile/splice on a plaintext TCP connection for `*os.File` and socket sources) the copy is delegated to it; otherwise — a compressing or other wrapping writer, HTTP/2 — the copy runs through a framework-pooled 32 KiB buffer and every chunk passes through `Response.Write`. A source implementing `io.WriterTo` writes itself in both cases. The bytes the writer accepted count toward `Size()`, a partial-write error is returned with the accepted count, a hijacked response answers `http.ErrHijacked`, and nothing is committed before the first byte: a source that fails up front leaves the response uncommitted so the handler's error still renders, and the first byte commits status 200 when none was written — the same contract as `Write`.
