...
Decode failures get the same structured treatment before validation ever runs: a malformed body, a wrong-typed value, or trailing content after the JSON value returns a 400 response in the same `errors[]` shape, with a machine-readable reason code (`syntax`, `type_mismatch`, `invalid_value`, `empty_body`, `trailing_data`). See the [Context Spec](../specs/context.md) for the decode error model.
