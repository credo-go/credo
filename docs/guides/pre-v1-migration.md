...
| `BuildURI`/`BuildURL` pasted values verbatim | Values are validated against their constraints and percent-encoded per segment (`a/b` becomes `a%2Fb`); static text is written in its wire spelling; host labels are validated, never encoded; empty values, invalid UTF-8 and a value containing its parameter's delimiter are errors |
| Static text with non-ASCII bytes (`/café`) was unreachable through `EscapedPath` matching | Static text and the request meet in one canonical form: every escape except `%2F`/`%25` is decoded before matching, so any spelling of the same bytes routes alike and an encoded delimiter is the delimiter |
