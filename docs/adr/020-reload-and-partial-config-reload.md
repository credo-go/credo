...
`Run()` registers SIGHUP alongside SIGINT/SIGTERM. A SIGHUP calls `app.Reload` with a context bounded by `WithReloadTimeout` (default 30s; also settable through the `server.reload_timeout` config key, mirroring `server.shutdown_timeout`). Signals that arrive while a reload is in flight are coalesced into at most one follow-up reload (buffered channel of one). SIGHUP never terminates the process, and a failed reload never terminates the process either: a service that keeps running on its previous configuration is strictly better than one that dies because an operator fat-fingered a YAML file.
...
`app.Reload` succeeds only in the `running` state. Before `running` it returns an error (nothing to reload); during `stopping`/`stopped` it returns `errReloadNotRunning` (mapped to a no-op for the signal path). Concurrent calls are serialized by a mutex; the second caller waits and then performs its own full reload, so the observable guarantee is "after my `Reload` returns, the snapshot is at least as new as when I called".
