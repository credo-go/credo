...
**Implemented 2026-09-05 (wire minor; closes G3).** The router matches the canonical form of the wire path — `URL.EscapedPath()` with every escape decoded except those of the RFC 3986 reserved characters and of `%` — so that segment boundaries and reserved spellings come from the client while every equivalent spelling reaches the same route (`/caf%C3%A9`, `/caf%c3%a9` and `/%63af%C3%A9` meet static text registered as `/café`; an encoded unreserved delimiter is the delimiter, since `%2D` and `-` are one URI under RFC 3986 §2.3, while `%3B` and `;` stay two URIs under §2.2 and a captured `%3B` decodes to `;`). It decodes each captured value exactly once and evaluates regex constraints on the decoded value that `RouteParam` reports. A captured `%2F` is data within its segment; `%252F` becomes literal `%2F`, not a slash. `%31` matches a numeric constraint as `1`; `+` stays plus and valid Unicode is preserved. A parameter candidate ends at its tail byte or the next raw slash, whichever comes first, so `{name}` and `{name:regex}` are single-segment and `{name...}` is the only way to capture several segments (with its slashes kept and each segment decoded).

Generation accepts decoded values, validates regex constraints, rejects invalid UTF-8 and a value containing its parameter's delimiter (neither could route back), uses `PathEscape` per segment and writes static text in its wire spelling; catch-all slash separators are retained and host labels are validated (letters, digits, hyphens, underscores, plus the label's constraint) rather than percent-encoded. Round trips preserve parameter values rather than identical percent-encoded spelling. A candidate that decodes to invalid UTF-8 answers 400 (`invalid_path_encoding`) when no route matches; malformed percent-encoding is rejected by net/http before dispatch; a valid value failing a regex is a route non-match. Generation rejects empty values, constraint failures and invalid host labels with an error.

Decoding the whole path before routing was rejected: an encoded slash would create a segment the client never sent, and there is no way back. Matching on the raw path alone was tried first and rejected too: it made static text with non-ASCII bytes unreachable and let two spellings of one URI route differently. The canonical form keeps exactly the escapes RFC 3986 §2.2 protects from normalization — the reserved characters, the slash among them — plus the percent sign against double decoding, and decodes everything else (§2.3), so the client's boundaries and reserved spellings survive, static text registered as written is met by every equivalent spelling, and a path without escapes costs nothing. Decoding the reserved characters too was tried and rejected: `/api/a%3Bb` reached a mounted app as `/a;b`, a different URI. A candidate is cut on escape units, so a `%` after a parameter (`/x/{id}%done`) is found only as its `%25` unit and never inside a captured `%2F`. Applying the constraint to the whole decoded value replaced Chi's prefix match (`^(re)` on the raw candidate): a constraint can no longer consume part of a segment or span a raw slash, and a tail-bounded parameter (`{name}.json`) is single-segment too — the grammar says a parameter is one segment, and the old behaviour made `/{name}.json` accept `/a/b.json`. Rewrite targets and mount handoffs are wire-form for the same reason: a percent-encoded value must survive as one parameter through an internal re-dispatch or a nested app. The [router spec](../specs/router.md#encoded-parameter-values) holds the positive and negative cases. This amendment shipped in its own wire-contract minor, independently from the endpoint-owned parameter names of the router minor (2026-09-05).
