...
**Status:** Accepted, implemented in v0.20.0; [restart backoff](#restart-backoff) accepted 2026-09-19, implemented in v0.21.0 **Date:** 2026-09-19 **Depends on:** ADR-004, ADR-006, ADR-016, ADR-022 **Related:** ADR-021 **Specification:** [Worker spec](../specs/worker.md) **Guide:** [Worker guide](../guides/worker.md)

**2026-09-19 amendment (v0.21.0):** continuous workers restart with a capped, jittered exponential delay by default. The decision is recorded under [Restart backoff](#restart-backoff) and in the alternatives below.
...
`k` is the failure's position in the current sequence, starting at 1, and the multiplier is fixed at 2. `base` is the effective restart delay (`WithRestartDelay` → `worker.restart_delay` → `DefaultRestartDelay`); it is the first and the minimum delay. The cap is `WithMaxRestartDelay(d)` per worker, `worker.max_restart_delay` per pool, `DefaultMaxRestartDelay` = 1 minute otherwise.
...
The cap resolves the way the base does. An omitted option takes the pool configuration, then the default, and is raised to `base` when `base` is larger: a pool-level cap below one worker's base is a default that does not fit that worker, not an error. An explicit `WithMaxRestartDelay(0)` skips the pool configuration and selects `max(DefaultMaxRestartDelay, base)`. An explicit positive cap is used as given, and one below the effective `base` is a registration error. A `WithRestartDelay(10 * time.Minute)` alone is therefore valid, and a fixed delay unless a pool cap above ten minutes is configured. The only fixed delay that no configuration can change is the same positive value in both options.
...
The v0.21.0 restart backoff is a behavior change without a compile error. Restarts move further apart after repeated failures, so a worker with `WithMaxRestarts(N)` reaches `failed` — and a `FailWhenFailed` readiness check drops — later than with a fixed delay: with the defaults, `WithMaxRestarts(5)` waits roughly 48–93 s in total instead of 15 s. Setting both delay options to the same value restores the fixed delay. The v0.21.0 release notes and the [migration guide](../guides/pre-v1-migration.md#workers) carry both rows.
