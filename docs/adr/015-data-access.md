...
**MySQL projection names fail loud.** MySQL requires every derived-table output name to be unique. After model hooks, Credo renders the MySQL count source once and accepts only provable simple qualified names or raw expressions with unique portable ASCII `AS` aliases. The proof parses those exact bytes under both normal backslash escaping and `NO_BACKSLASH_ESCAPES`, because session mode must not hide a projection separator or executable comment. Duplicate names, wildcards, implicit aliases, and unprovable names return `ErrUnsupportedCountQuery` before I/O; PostgreSQL and SQLite are not narrowed by this guard. A derived-column alias list was rejected because Credo does not know the MySQL/MariaDB server capability/version and the list requires exact projection arity. See the [MySQL derived-table contract](https://dev.mysql.com/doc/mysql/en/derived-tables.html).
