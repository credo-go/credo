...
Credo is **Beta** overall. Shipped packages are usable for real development; the table below is explicit about what has shipped and what is still on the roadmap. Planned areas exist only as roadmap entries in [TODO.md](TODO.md); the module contains no placeholder directories, so nothing is importable before real code lands. Every importable package repeats its label as a machine-readable `Maturity: <label>` line at the end of its package documentation (`go doc <pkg>`), and `maturity_test.go` checks that those lines agree with this table.
...
**Bootstrap/DI (DI minor), router parameter-name (router minor), built-in HTTP feature (HTTP minor) and URL round-trip (wire minor) changes implemented 2026-09-05.** All contracts are recorded in the [migration guide](docs/guides/pre-v1-migration.md) and the [implementation plan](docs/plans/pre-v1-implementation.md); progress lives in [TODO.md](TODO.md#pre-v1-contract-migration). The quick start and examples below use the shipped APIs, and the [example migration notes](examples/README.md) explain what changed in them.
